class HomeController < ActionController::Base
  ...
  def index
    ...
    if params[:category_id] && params[:category_id].to_i == 1
      ...
      if page > 1
        ...
        merge_infos = (data[:infos].sample(5) + today_infos.sample(15)).uniq.sample(10)

        today_videos = Video.today_list
        merge_videos = (data[:videos].sample(5) + today_videos.sample(15)).uniq.sample(10)
      else
  ...
  def location_source
    ...
    flow_medias = {}
    flow_medias[:videos] = []
    Video.is_ads.enabled.order(:ads_index).each do |_|
  ...
  def youtube_share
    ...
    if !data.present?
      data = {}  
    elsif JSON.parse(data)["status"] != 1
      data = {}
    end
  ...
  def youtube_catch
    ...
    unless data.present?
      ...
      uuid = params[:uuid]
      # 是否有本地缓存
      @video = Video.where("location_source_url is not null and url like '%?' ",link.split("//").last).last  if link.split("//")&.last.present?
      local_id = @video.present? ? 1 : 0

      data = {"uuid": uuid ,"title": title,"link": link,"image_base64": image_base64, "local_model": "Video", "medial_source": "youtube", "local_id": local_id, "status":0, "location_source_url": @video&.location_source_url}.to_json 
      $redis.set(key,data)
      # 添加审核列表
  ...
  def old_index
    ...
    @info_tops.each do |_|
      flow_medias[:tops] << {medial_type: "info", medial_id:_.id, title:_.title,url:_.url,mobile_url:_.mobile_url,local_image_url:_.local_image_url,tag_ids:_.tag_ids.join(","), web_target:_.spider_target&.name, web_target_logo: _.spider_target&.logo_url }
    end
    # 强推信息
    @info_forces.each do |_|
      flow_medias[:infos] << {medial_type: "info", medial_id:_.id, title:_.title,url:_.url,mobile_url:_.mobile_url,local_image_url:_.local_image_url,tag_ids:_.tag_ids.join(","), web_target:_.spider_target&.name, web_target_logo: _.spider_target&.logo_url }
    end
    # 视频
    @videos.each do |_|
      flow_medias[:videos] << {author:_.author,medial_type: "video", medial_id:_.id, title:_.title,url:_.url,mobile_url:_.mobile_url,local_image_url:_.local_image_url,tag_ids:_.tag_ids.join(","), web_target:_.spider_target&.name, web_target_logo: _.spider_target&.logo_url,overlay_time: _.overlay_time, play_count:_.play_count }
